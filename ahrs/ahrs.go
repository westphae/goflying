...
func (s *State) calcJacobianState(c Control) matrix.DenseMatrix {
	...
	data[0][0] = 1                                         // U1,U1
	data[0][1] = dt * (-h3)  // U1,U2
	data[0][2] = dt * (h2) // U1,U3
	data[0][3] = dt * (-2*G*s.E2)  // U1/E0
	data[0][4] = dt * (-2*G*s.E3)   // U1/E1
	data[0][5] = dt * (-2*G*s.E0)              // U1/E2
	data[0][6] = dt * (-2*G*s.E1)               // U1/E3
	data[1][0] = dt * (h3) // U2/U1
	data[1][1] = 1                                         // U2/U2
	data[1][2] = dt * (-h1)  // U2/U3
	data[1][3] = dt * (+2*G*s.E1)   // U2/E0
	data[1][4] = dt * (+2*G*s.E0)               // U2/E1
	data[1][5] = dt * (-2*G*s.E3)   // U2/E2
	data[1][6] = dt * (-2*G*s.E2)               // U2/E3
	data[2][0] = dt * (-h2)  // U3/U1
	data[2][1] = dt * (h1) // U3/U2
	data[2][2] = 1                                         // U3/U3
	data[2][3] = dt * (-4*G*s.E0)   // U3/E0
	data[2][4] = dt * (-4*G*s.E1)              // U3/E1
	data[2][5] = dt * (-4*G*s.E2)              // U3/E2
	data[2][6] = dt * (-4*G*s.E3)   // U3/E3
	data[3][3] = 1                                         // E0/E0
